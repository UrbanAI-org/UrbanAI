...
from src.query.chunks import Chunk
import base64
"""
!!!
...
class V1QueryChunks(Resource):
    def get(self):
        ...
        """
        data = database.execute_in_worker(qry)
        result = []
        for each in data:
            result.append({
                "id" : each[0],
                'herf': f"/v1/query/chunk?id={each[0]}"
            })
        return {
            "chunks": result}
    ...
    def post(self):
        ...
        if data['type'] == 'polygon':
            chunk = Chunk.create_by_polygon(data['data']['polygon'], tif[1:], tif[0])
        elif data['type'] == 'circle':
            chunk = Chunk.create_by_circle(data['data']['center'], data['data']['radius'], tif[1:], tif[0])
        else:
...
class V1QueryChunk(Resource):
    def get(self):
        ...
        chunk = Chunk.read_from_database(id)
        if chunk is None:
            return {}
...
class V1Resource(Resource):
    ...
    def post(self):
        ...
        chunk = Chunk.read_from_database(data['chunk_id'])
        if chunk is None:
        ...
        return {
            'id' : pcd_resource_id,
            'herf': f"/v1/resource?id={pcd_resource_id}&type={data['type']}",
            'expired' : 3
        }  
